            ...
            res.append(
                min(
                    [
                        (pi * P_fn + (1 - pi) * P_fp)
                        / (min(pi * Cf_n, (1 - pi) * Cf_p) if normalize else 1)
                        for P_fn, P_fp in zip(P_fn_Cf_n, P_fp_Cf_p)
                    ]
                )
            )
    else:
...
def bayes_error_plot(
    ...
    act_dcf = dcf_range(scores, labels, effective_priors, 1, 1, "optimal")
    min_dcf = dcf_range(scores, labels, effective_priors, 1, 1, "min")
